...
- [ ] GitHub account that can create **github.com/r-medina/scviz** (or another name)
...
```bash
gh repo create r-medina/scviz --public --source=. --remote=origin --push
```

Or on github.com: **New repository** → public → empty (no README) → then:

```bash
git remote add origin git@github.com:r-medina/scviz.git
...
If the GitHub name is not `r-medina/scviz`, update the URLs in `PRIVACY.md`, `SECURITY.md`, and `store/LISTING.md` before submitting the store listing.
...
That writes `store/dist/scviz.zip`. Unzip it somewhere and **Load unpacked** that folder once as a sanity check.
...
2. **New item** → upload `store/dist/scviz.zip`
3. **Store listing** — paste from `store/LISTING.md`  
   Upload `icons/icon128.png`, `store/promo/small-tile.png`, `store/promo/marquee.png`, and the four `store/screenshots/*.png`
...
6. Take **one real screenshot on soundcloud.com** (play bar button + overlay) and add it. Reviewers like seeing the extension in place.
7. Submit for review
